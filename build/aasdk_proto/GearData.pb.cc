// Generated by the protocol buffer compiler.  DO NOT EDIT!
// source: GearData.proto

#include "GearData.pb.h"

#include <algorithm>

#include <google/protobuf/io/coded_stream.h>
#include <google/protobuf/extension_set.h>
#include <google/protobuf/wire_format_lite.h>
#include <google/protobuf/descriptor.h>
#include <google/protobuf/generated_message_reflection.h>
#include <google/protobuf/reflection_ops.h>
#include <google/protobuf/wire_format.h>
// @@protoc_insertion_point(includes)
#include <google/protobuf/port_def.inc>

PROTOBUF_PRAGMA_INIT_SEG

namespace _pb = ::PROTOBUF_NAMESPACE_ID;
namespace _pbi = _pb::internal;

namespace f1x {
namespace aasdk {
namespace proto {
namespace data {
PROTOBUF_CONSTEXPR Gear::Gear(
    ::_pbi::ConstantInitialized): _impl_{
    /*decltype(_impl_.gear_)*/0
  , /*decltype(_impl_._cached_size_)*/{}} {}
struct GearDefaultTypeInternal {
  PROTOBUF_CONSTEXPR GearDefaultTypeInternal()
      : _instance(::_pbi::ConstantInitialized{}) {}
  ~GearDefaultTypeInternal() {}
  union {
    Gear _instance;
  };
};
PROTOBUF_ATTRIBUTE_NO_DESTROY PROTOBUF_CONSTINIT PROTOBUF_ATTRIBUTE_INIT_PRIORITY1 GearDefaultTypeInternal _Gear_default_instance_;
}  // namespace data
}  // namespace proto
}  // namespace aasdk
}  // namespace f1x
static ::_pb::Metadata file_level_metadata_GearData_2eproto[1];
static constexpr ::_pb::EnumDescriptor const** file_level_enum_descriptors_GearData_2eproto = nullptr;
static constexpr ::_pb::ServiceDescriptor const** file_level_service_descriptors_GearData_2eproto = nullptr;

const uint32_t TableStruct_GearData_2eproto::offsets[] PROTOBUF_SECTION_VARIABLE(protodesc_cold) = {
  ~0u,  // no _has_bits_
  PROTOBUF_FIELD_OFFSET(::f1x::aasdk::proto::data::Gear, _internal_metadata_),
  ~0u,  // no _extensions_
  ~0u,  // no _oneof_case_
  ~0u,  // no _weak_field_map_
  ~0u,  // no _inlined_string_donated_
  PROTOBUF_FIELD_OFFSET(::f1x::aasdk::proto::data::Gear, _impl_.gear_),
};
static const ::_pbi::MigrationSchema schemas[] PROTOBUF_SECTION_VARIABLE(protodesc_cold) = {
  { 0, -1, -1, sizeof(::f1x::aasdk::proto::data::Gear)},
};

static const ::_pb::Message* const file_default_instances[] = {
  &::f1x::aasdk::proto::data::_Gear_default_instance_._instance,
};

const char descriptor_table_protodef_GearData_2eproto[] PROTOBUF_SECTION_VARIABLE(protodesc_cold) =
  "\n\016GearData.proto\022\024f1x.aasdk.proto.data\032\016"
  "GearEnum.proto\"6\n\004Gear\022.\n\004gear\030\001 \001(\0162 .f"
  "1x.aasdk.proto.enums.Gear.Enumb\006proto3"
  ;
static const ::_pbi::DescriptorTable* const descriptor_table_GearData_2eproto_deps[1] = {
  &::descriptor_table_GearEnum_2eproto,
};
static ::_pbi::once_flag descriptor_table_GearData_2eproto_once;
const ::_pbi::DescriptorTable descriptor_table_GearData_2eproto = {
    false, false, 118, descriptor_table_protodef_GearData_2eproto,
    "GearData.proto",
    &descriptor_table_GearData_2eproto_once, descriptor_table_GearData_2eproto_deps, 1, 1,
    schemas, file_default_instances, TableStruct_GearData_2eproto::offsets,
    file_level_metadata_GearData_2eproto, file_level_enum_descriptors_GearData_2eproto,
    file_level_service_descriptors_GearData_2eproto,
};
PROTOBUF_ATTRIBUTE_WEAK const ::_pbi::DescriptorTable* descriptor_table_GearData_2eproto_getter() {
  return &descriptor_table_GearData_2eproto;
}

// Force running AddDescriptors() at dynamic initialization time.
PROTOBUF_ATTRIBUTE_INIT_PRIORITY2 static ::_pbi::AddDescriptorsRunner dynamic_init_dummy_GearData_2eproto(&descriptor_table_GearData_2eproto);
namespace f1x {
namespace aasdk {
namespace proto {
namespace data {

// ===================================================================

class Gear::_Internal {
 public:
};

Gear::Gear(::PROTOBUF_NAMESPACE_ID::Arena* arena,
                         bool is_message_owned)
  : ::PROTOBUF_NAMESPACE_ID::Message(arena, is_message_owned) {
  SharedCtor(arena, is_message_owned);
  // @@protoc_insertion_point(arena_constructor:f1x.aasdk.proto.data.Gear)
}
Gear::Gear(const Gear& from)
  : ::PROTOBUF_NAMESPACE_ID::Message() {
  Gear* const _this = this; (void)_this;
  new (&_impl_) Impl_{
      decltype(_impl_.gear_){}
    , /*decltype(_impl_._cached_size_)*/{}};

  _internal_metadata_.MergeFrom<::PROTOBUF_NAMESPACE_ID::UnknownFieldSet>(from._internal_metadata_);
  _this->_impl_.gear_ = from._impl_.gear_;
  // @@protoc_insertion_point(copy_constructor:f1x.aasdk.proto.data.Gear)
}

inline void Gear::SharedCtor(
    ::_pb::Arena* arena, bool is_message_owned) {
  (void)arena;
  (void)is_message_owned;
  new (&_impl_) Impl_{
      decltype(_impl_.gear_){0}
    , /*decltype(_impl_._cached_size_)*/{}
  };
}

Gear::~Gear() {
  // @@protoc_insertion_point(destructor:f1x.aasdk.proto.data.Gear)
  if (auto *arena = _internal_metadata_.DeleteReturnArena<::PROTOBUF_NAMESPACE_ID::UnknownFieldSet>()) {
  (void)arena;
    return;
  }
  SharedDtor();
}

inline void Gear::SharedDtor() {
  GOOGLE_DCHECK(GetArenaForAllocation() == nullptr);
}

void Gear::SetCachedSize(int size) const {
  _impl_._cached_size_.Set(size);
}

void Gear::Clear() {
// @@protoc_insertion_point(message_clear_start:f1x.aasdk.proto.data.Gear)
  uint32_t cached_has_bits = 0;
  // Prevent compiler warnings about cached_has_bits being unused
  (void) cached_has_bits;

  _impl_.gear_ = 0;
  _internal_metadata_.Clear<::PROTOBUF_NAMESPACE_ID::UnknownFieldSet>();
}

const char* Gear::_InternalParse(const char* ptr, ::_pbi::ParseContext* ctx) {
#define CHK_(x) if (PROTOBUF_PREDICT_FALSE(!(x))) goto failure
  while (!ctx->Done(&ptr)) {
    uint32_t tag;
    ptr = ::_pbi::ReadTag(ptr, &tag);
    switch (tag >> 3) {
      // .f1x.aasdk.proto.enums.Gear.Enum gear = 1;
      case 1:
        if (PROTOBUF_PREDICT_TRUE(static_cast<uint8_t>(tag) == 8)) {
          uint64_t val = ::PROTOBUF_NAMESPACE_ID::internal::ReadVarint64(&ptr);
          CHK_(ptr);
          _internal_set_gear(static_cast<::f1x::aasdk::proto::enums::Gear_Enum>(val));
        } else
          goto handle_unusual;
        continue;
      default:
        goto handle_unusual;
    }  // switch
  handle_unusual:
    if ((tag == 0) || ((tag & 7) == 4)) {
      CHK_(ptr);
      ctx->SetLastTag(tag);
      goto message_done;
    }
    ptr = UnknownFieldParse(
        tag,
        _internal_metadata_.mutable_unknown_fields<::PROTOBUF_NAMESPACE_ID::UnknownFieldSet>(),
        ptr, ctx);
    CHK_(ptr != nullptr);
  }  // while
message_done:
  return ptr;
failure:
  ptr = nullptr;
  goto message_done;
#undef CHK_
}

uint8_t* Gear::_InternalSerialize(
    uint8_t* target, ::PROTOBUF_NAMESPACE_ID::io::EpsCopyOutputStream* stream) const {
  // @@protoc_insertion_point(serialize_to_array_start:f1x.aasdk.proto.data.Gear)
  uint32_t cached_has_bits = 0;
  (void) cached_has_bits;

  // .f1x.aasdk.proto.enums.Gear.Enum gear = 1;
  if (this->_internal_gear() != 0) {
    target = stream->EnsureSpace(target);
    target = ::_pbi::WireFormatLite::WriteEnumToArray(
      1, this->_internal_gear(), target);
  }

  if (PROTOBUF_PREDICT_FALSE(_internal_metadata_.have_unknown_fields())) {
    target = ::_pbi::WireFormat::InternalSerializeUnknownFieldsToArray(
        _internal_metadata_.unknown_fields<::PROTOBUF_NAMESPACE_ID::UnknownFieldSet>(::PROTOBUF_NAMESPACE_ID::UnknownFieldSet::default_instance), target, stream);
  }
  // @@protoc_insertion_point(serialize_to_array_end:f1x.aasdk.proto.data.Gear)
  return target;
}

size_t Gear::ByteSizeLong() const {
// @@protoc_insertion_point(message_byte_size_start:f1x.aasdk.proto.data.Gear)
  size_t total_size = 0;

  uint32_t cached_has_bits = 0;
  // Prevent compiler warnings about cached_has_bits being unused
  (void) cached_has_bits;

  // .f1x.aasdk.proto.enums.Gear.Enum gear = 1;
  if (this->_internal_gear() != 0) {
    total_size += 1 +
      ::_pbi::WireFormatLite::EnumSize(this->_internal_gear());
  }

  return MaybeComputeUnknownFieldsSize(total_size, &_impl_._cached_size_);
}

const ::PROTOBUF_NAMESPACE_ID::Message::ClassData Gear::_class_data_ = {
    ::PROTOBUF_NAMESPACE_ID::Message::CopyWithSourceCheck,
    Gear::MergeImpl
};
const ::PROTOBUF_NAMESPACE_ID::Message::ClassData*Gear::GetClassData() const { return &_class_data_; }


void Gear::MergeImpl(::PROTOBUF_NAMESPACE_ID::Message& to_msg, const ::PROTOBUF_NAMESPACE_ID::Message& from_msg) {
  auto* const _this = static_cast<Gear*>(&to_msg);
  auto& from = static_cast<const Gear&>(from_msg);
  // @@protoc_insertion_point(class_specific_merge_from_start:f1x.aasdk.proto.data.Gear)
  GOOGLE_DCHECK_NE(&from, _this);
  uint32_t cached_has_bits = 0;
  (void) cached_has_bits;

  if (from._internal_gear() != 0) {
    _this->_internal_set_gear(from._internal_gear());
  }
  _this->_internal_metadata_.MergeFrom<::PROTOBUF_NAMESPACE_ID::UnknownFieldSet>(from._internal_metadata_);
}

void Gear::CopyFrom(const Gear& from) {
// @@protoc_insertion_point(class_specific_copy_from_start:f1x.aasdk.proto.data.Gear)
  if (&from == this) return;
  Clear();
  MergeFrom(from);
}

bool Gear::IsInitialized() const {
  return true;
}

void Gear::InternalSwap(Gear* other) {
  using std::swap;
  _internal_metadata_.InternalSwap(&other->_internal_metadata_);
  swap(_impl_.gear_, other->_impl_.gear_);
}

::PROTOBUF_NAMESPACE_ID::Metadata Gear::GetMetadata() const {
  return ::_pbi::AssignDescriptors(
      &descriptor_table_GearData_2eproto_getter, &descriptor_table_GearData_2eproto_once,
      file_level_metadata_GearData_2eproto[0]);
}

// @@protoc_insertion_point(namespace_scope)
}  // namespace data
}  // namespace proto
}  // namespace aasdk
}  // namespace f1x
PROTOBUF_NAMESPACE_OPEN
template<> PROTOBUF_NOINLINE ::f1x::aasdk::proto::data::Gear*
Arena::CreateMaybeMessage< ::f1x::aasdk::proto::data::Gear >(Arena* arena) {
  return Arena::CreateMessageInternal< ::f1x::aasdk::proto::data::Gear >(arena);
}
PROTOBUF_NAMESPACE_CLOSE

// @@protoc_insertion_point(global_scope)
#include <google/protobuf/port_undef.inc>
