...
{
    strand_.post([this, self = this->shared_from_this(), promise = std::move(promise)]() {
...
void USBHub::cancel()
{
    strand_.post([this, self = this->shared_from_this()]() mutable {
        if(hotplugPromise_ != nullptr)
        {
            hotplugPromise_->reject(error::Error(error::ErrorCode::OPERATION_ABORTED));
            hotplugPromise_.reset();
...
int USBHub::hotplugEventsHandler(libusb_context* usbContext, libusb_device* device, libusb_hotplug_event event, void* userData)
{
    if(event == LIBUSB_HOTPLUG_EVENT_DEVICE_ARRIVED)
    {
        ...
        self->strand_.post(std::bind(&USBHub::handleDevice, self, device));
    }
    
    return 0;
}

bool USBHub::isAOAPDevice(libusb_device_descriptor  deviceDescriptor) const
{
