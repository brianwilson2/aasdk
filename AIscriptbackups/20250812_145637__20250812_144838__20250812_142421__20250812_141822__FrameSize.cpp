...
FrameSize::FrameSize(const common::DataConstBuffer& buffer)
{
    ...
    {
        frameSizeType_ = FrameSizeType::SHORT;
        frameSize_ = boost::endian::big_to_native(reinterpret_cast<uint16_t >(buffer.cdata[0]));
    }

    if(buffer.size >= 6)
    {
        frameSizeType_ = FrameSizeType::EXTENDED;
        totalSize_ = boost::endian::big_to_native(reinterpret_cast<uint32_t >(buffer.cdata[2]));
    }
}
