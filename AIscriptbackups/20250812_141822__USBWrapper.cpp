...
int USBWrapper::releaseInterface(DeviceHandle  dev_handle, int interface_number)
{
    return libusb_release_interface(dev_handle.get(), interface_number);
}

libusb_device* USBWrapper::getDevice(DeviceHandle  dev_handle)
{
    return libusb_get_device(dev_handle.get());
}

int USBWrapper::claimInterface(DeviceHandle  dev_handle, int interface_number)
{
