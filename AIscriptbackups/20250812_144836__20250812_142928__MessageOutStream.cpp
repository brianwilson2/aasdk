...
{
    strand_.post([this, self = this->shared_from_this(), message = std::move(message), promise = std::move(promise)]() mutable {
        if(promise_ != nullptr)
        {
...
void MessageOutStream::streamSplittedMessage()
{
    try
    {
        auto  payload = message_->getPayload();
...
{
    auto  frameSize = frameType == FrameType::FIRST ? FrameSize(payloadSize, totalSize) : FrameSize(payloadSize);
    auto  frameSizeData = frameSize.getData();
